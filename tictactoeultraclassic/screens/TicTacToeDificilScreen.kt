...
@Composable
fun TicTacToeScreenDificil(navController: NavHostController, nombre: String, fichaJugador: String) {
        ...
        if (gameState.value[i][j] == null && !buttonsDisabled.value) {
            ...
            if (isFirstMove.value) {
                ...
                if (gameState.value[1][1] == null) {
                    ...
                    makeBestMoveDificil(gameState.value, machineFicha)
                }
            } else {
                makeBestMoveDificil(gameState.value, machineFicha)
            }
...
fun makeBestMoveDificil(board: Array<Array<String?>>, ficha: String) {
    ...
    val opponentFicha = if (ficha == "X") "O" else "X"
    for (i in 0..2) {
        for (j in 0..2) {
            if (board[i][j] == null) {
                board[i][j] = opponentFicha
                if (compruebaGanador(board, opponentFicha)) {
                    board[i][j] = ficha
                    return
                }
                board[i][j] = null
